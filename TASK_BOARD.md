...
- [x] Build&Test added `npm run webdev:extension:readiness` as a Gate D readiness-only static check: it verifies the legacy Browser Extension remains a deferred ecosystem component, has no WebDev replay / Cloudflare endpoint integration, and exposes no Extension deploy / CWS / release script.
