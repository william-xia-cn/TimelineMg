...
- [x] Build&Test advanced WebDev Google SSO session lifecycle: Worker supports `POST /auth/session/refresh` to rotate a valid TimeWhere bearer and revoke the old session without touching Google tokens; Pages Settings exposes Refresh session, editable TimeWhere workspace profile, and safe account/runtime/gate status diagnostics.
...
- [x] Build&Test added WebDev Phase 5 business parity checklist in `docs/WEBDEV_BUSINESS_PARITY_CHECKLIST.md`, mapping current Web App coverage, remaining non-gate work, and approval-gated gaps.
- [x] Build&Test advanced WebDev Phase 5 to local-preview coverage: Dashboard, Tasks, Calendar, Settings, Daily Settle projection, Reminder state, migration conflict review, structure editing, and Task-only pending diagnostics are covered by local implementation plus repeatable `npm run webdev:ui:walkthrough`; true preview cloud evidence remains Gate A / Phase 9.
- [x] Build&Test scaffolded WebDev Phase 7 Desktop Runtime opt-in mode: Electron can load a configured Web App URL with hash view routing and guarded navigation while preserving legacy extension shell as default; packaging/signing/distribution remains Gate E.
- [x] Build&Test added `docs/WEBDEV_BROWSER_EXTENSION_GATE_D_READINESS.md` and `npm run webdev:extension:readiness` as Gate D readiness-only evidence: it verifies the legacy Browser Extension remains a deferred ecosystem component, has no WebDev replay / Cloudflare endpoint integration, and exposes no Extension deploy / CWS / release script.
- [x] Build&Test added WebDev Phase 9 preview acceptance runbook and Phase 10 prod readiness checklist: `docs/WEBDEV_PREVIEW_ACCEPTANCE_RUNBOOK.md` and `docs/WEBDEV_PROD_READINESS_CHECKLIST.md`; real preview resources remain Gate A and prod/release remains Gate R.
...
- [x] Build&Test added Cloud revision / change cursor foundation while keeping replay writes blocked: D1 `sync_changes`, Worker `/sync/bootstrap`, Worker `/sync/changes`, repository change recording, scaffold tests, and local Worker integration coverage.
- [x] Build&Test wired Web App read-cache refresh to `/sync/bootstrap` plus cursor-based `/sync/changes`: Tasks / Calendar / Structure / Settings caches can consume Cloud-confirmed changes, and pending local Task rows are preserved until retry/discard/replay decisions.
- [x] Build&Test strengthened local preview evidence: `npm run webdev:ui:walkthrough` now opens Settings, verifies the read-cache cursor, creates a local Worker Cloud Task after Web App bootstrap, clicks `Refresh changes`, and verifies the new Task reaches the Tasks UI via `/sync/changes` without creating Cloudflare resources or enabling replay writes.
- [x] Build&Test added local Desktop Runtime evidence without Gate E packaging: `npm run webdev:desktop:smoke` starts local Worker/Pages and launches Electron in WebDev runtime smoke mode.
- [x] Build&Test added `docs/WEBDEV_DESKTOP_RUNTIME_GATE_E_READINESS.md` and `npm run webdev:desktop:readiness` as Gate E readiness-only evidence: it verifies Electron WebDev runtime mode, route guards, native preload bridge, docs, and packaging boundary without launching Electron, building a package, signing, notarizing, or distributing.
- [x] Build&Test added `npm run webdev:acceptance:local` as the local non-release acceptance command combining plan/API/build/integration checks, UI walkthrough, and Desktop Runtime smoke.
- [x] Build&Test added `npm run webdev:preview:preflight` as the Gate A preflight command: it verifies preview/prod readiness boundaries, placeholder resource ids, replay kill switches, env examples, and sensitive-info hygiene without creating Cloudflare resources or deploying.
- [x] Build&Test added Gate A execution commands `npm run webdev:cloudflare:provision` and `npm run webdev:preview:deploy`: provision creates/confirms dev/preview resources only and stores true ids under ignored `.wrangler/`; preview deploy applies preview D1 migrations and deploys preview Worker/Pages using generated local config.
- [x] Product Owner approved Gate A for dev/preview resource creation. Build&Test created/confirmed `timewhere-dev-*` and `timewhere-preview-*` Cloudflare D1/R2/KV resources plus dev/preview Pages projects; true resource ids are stored only under ignored `.wrangler/`.
- [x] Build&Test deployed the Gate A preview Worker and stable Pages preview using the preview Google Web OAuth public client id provided through shell environment only. Stable preview Google SSO smoke passes: Settings shows `Google SSO connected`, the disconnected banner is gone, and the Worker-backed Cloud session is active. OAuth secret, account email, tokens, Cloudflare ids, and private local paths were not recorded in repo evidence.
- [x] Build&Test added and ran `npm run webdev:preview:smoke`: preview Worker `/health`, stable Pages load, remote D1 core table query, preview R2 temporary object write/read/delete, and preview KV temporary key write/read/delete all pass. The script refuses non-preview resource names and leaves true ids in ignored `.wrangler/` state only.
- [x] Build&Test recorded Phase 9 supporting evidence: `npm run webdev:verify`, `npm run webdev:ui:walkthrough`, `npm run webdev:desktop:smoke`, full `npm test`, `git diff --check`, and repo-scoped sensitive pattern scan all pass. Preview core API smoke covers the core Cloud data path, and preview UI smoke covers stable Pages preview UI with temporary smoke data.
- [x] Build&Test added and ran `npm run webdev:preview:core-smoke`: using a temporary smoke account/session, preview Worker API Account status/profile, Structure create, Task create/update/list, Calendar create/update/list, Settings update, Sync bootstrap, Sync changes, Migration import, idempotent retry, migration conflict generation, and conflict resolution all pass; smoke entities and temporary migration snapshots are cleaned up and final smoke account count is 0. No prod resources, Google session, account email, token, or Cloudflare id were recorded.
- [x] Build&Test added and ran `npm run webdev:preview:ui-smoke`: it uses a temporary smoke account/session to open stable Pages preview and validate Dashboard, Tasks, Calendar, and Settings UI against preview Worker data, then cleans up the smoke account. No prod resources, Google browser session, account email, token, or Cloudflare id are recorded.
- [x] Build&Test added `npm run webdev:preview:acceptance` as the Gate A preview acceptance aggregator: it runs foundation preview smoke, core API smoke, and UI smoke in sequence without touching prod.
- [x] Build&Test reran `npm run webdev:preview:acceptance` after the WebDev completion audit / docs cleanup work: headers, preview resource, core API, UI, and data hygiene checks all passed; no prod resources, tokens, account emails, Cloudflare ids, or private local paths were printed.
- [x] Build&Test added Cloudflare Pages security headers and cache policy in `pages/public/_headers`, with scaffold/preflight/prod-readiness checks confirming CSP, basic security headers, immutable asset cache, and `index.html` no-store without embedding secrets or Cloudflare ids.
- [x] Build&Test expanded Phase 9/10 readiness documentation with preview risk register, known limitations, preview rollback/cleanup plan, and Gate R release risk register; scaffold/preflight/prod-readiness checks now assert these sections remain present.
- [x] Build&Test deployed the updated preview Pages headers and added `npm run webdev:preview:headers-smoke`, which verifies stable preview CSP/security headers, root HTML no-store, and hashed asset immutable cache before the rest of preview acceptance.
- [x] Build&Test added `npm run webdev:preview:data-hygiene-smoke`, which verifies preview D1 has no smoke account/entity/migration references, preview KV has no smoke temporary keys, and local `.wrangler` has no smoke temp files after preview acceptance.
- [x] Build&Test added `docs/WEBDEV_OBSERVABILITY_BACKUP_RUNBOOK.md` and `npm run webdev:observability:readiness` as Gate R readiness-only evidence for Worker error envelope, migration/sync conflict privacy guards, D1/R2 backup/restore planning, and stop conditions without calling Wrangler, exporting D1, reading R2, creating prod resources, or deploying.
- [x] Build&Test added `docs/WEBDEV_TASK_REPLAY_GATE_B_READINESS.md` and `npm run webdev:gate-b:readiness` as Gate B readiness-only evidence for Task replay approval; default replay remains disabled, test-only replay is dev/local/test only, non-Task replay remains blocked, and user-facing replay writes remain blocked.
- [x] Build&Test added `docs/WEBDEV_NON_TASK_REPLAY_GATE_C_READINESS.md` and `npm run webdev:gate-c:readiness` as Gate C readiness-only evidence for Calendar / Container / Settings replay approval; non-Task repositories still block offline writes and Worker replay still rejects non-Task mutations.
- [x] Build&Test added and ran `npm run webdev:prod:readiness` as a Gate R readiness-only static check: it verifies prod placeholders, Gate R boundary, replay kill switch, env example hygiene, and absence of exposed prod deploy/release scripts without creating prod resources or deploying.
- [x] Build&Test added `npm run webdev:prod:package` as a Gate R readiness package draft generator: it outputs branch/commit, Gate status, required evidence commands, optional Fresh Local Evidence Summary from ignored `.wrangler/webdev-gate-r-evidence-summary.json`, known limitations, and rollback summary without calling Wrangler, reading real Cloudflare resource state, creating prod resources, or deploying.
- [x] Build&Test added `npm run webdev:prod:evidence` as a Gate R local evidence runner: default mode is plan-only, `-- --run` executes required evidence commands and writes status/duration only to ignored `.wrangler/webdev-gate-r-evidence-summary.json`; it does not store raw output, create prod resources, deploy, publish, or enable replay.
- [x] Build&Test added `npm run webdev:prod:evidence:check` as a Gate R local evidence summary validator: it verifies the ignored summary matches the current clean `WebDev` HEAD and `origin/WebDev`, checks required command order and exit codes, confirms changed-files scan saw zero changed files, and rejects raw output fields without calling Wrangler or creating prod resources.
- [x] Build&Test fixed the WebDev Gate R evidence runner on Windows; `npm run webdev:prod:evidence -- --run` can produce fresh status-only evidence for the current clean pushed `WebDev` HEAD, and `npm run webdev:prod:evidence:check` verifies the ignored summary before Gate R review. This remains evidence only and does not approve Gate R.
- [x] Product Owner approved Gate R internal prod verification: create/confirm prod Cloudflare resources, deploy prod Worker / Pages, and run prod smoke/core/UI/data-hygiene evidence. This approval does not include public release, GitHub Release, tag, CWS, Desktop distribution, signing, notarization, or replay expansion.
- [x] Build&Test added and ran `webdev:prod:provision`, `webdev:prod:deploy`, and `webdev:prod:acceptance`: prod Cloudflare resources were created/confirmed, prod Worker / Pages deployed, `https://timewhere-web.pages.dev` passed headers, Google SSO button render, Worker / Pages / D1 / R2 / KV, core API, UI, and data hygiene checks with temporary `prod-smoke-*` data only.
- [x] Build&Test generated fresh Gate R evidence summary for the current clean pushed `WebDev` HEAD; `.wrangler/webdev-gate-r-evidence-summary.json` remains ignored and stores status/duration only, with no raw output or secrets.
- [x] Build&Test added `npm run webdev:completion:audit` as a current-state completion audit: it classifies WebDev as `readiness_complete_pending_approval_gates`, verifies Phase 0-10 evidence and Gate B/C/D/E/R boundaries, and does not approve prod, replay, desktop distribution, CWS, tag, merge, or release.
...
- [x] D-049 Phase 2-9 batch is complete. Await separate Product Owner approval before prod release, Calendar/Container/Settings replay implementation, Browser Extension replay, local-over-cloud overwrite, batch conflict handling, or full-entity offline-first.
