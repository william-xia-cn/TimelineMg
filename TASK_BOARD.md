...
- [x] Build&Test added `npm run webdev:prod:package` as a Gate R readiness package draft generator: it outputs branch/commit, Gate status, required evidence commands, known limitations, and rollback summary without reading `.wrangler/`, calling Wrangler, creating prod resources, or deploying.
...
- [x] Build&Test fixed the WebDev Gate R evidence runner on Windows and reran `npm run webdev:prod:evidence -- --run` on commit `ec82290`; all planned status-only evidence commands passed, including preview acceptance, local acceptance, full `npm test`, `git diff --check`, and changed-files sensitive scan. This remains evidence only and does not approve Gate R.
