...
This document defines shared rules for AI agents and developers working on `<PROJECT_NAME>`.

## 1. Lightweight Workflow

`<PROJECT_NAME>` is currently a `<PROJECT_STAGE>` project. Default workflow should stay lightweight and traceable.
...
- `docs/releases/*`: release evidence or release blockers only, not ordinary daily process.
