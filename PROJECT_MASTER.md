...
- D-030 / D-031 desktop preparation evidence:
  ...
  - Desktop OAuth client ID is bundled for the Windows app; `TIMEWHERE_GOOGLE_DESKTOP_CLIENT_ID` remains an optional override. Desktop OAuth is PKCE-only and must not read, send, store, or bundle a `client_secret`; if Google requires one, fix or recreate the Google Cloud OAuth client as a Desktop app client.
