...
- Current Desktop Runtime repositioning is opt-in scaffolded: Electron can load the Web App via `TIMEWHERE_DESKTOP_RUNTIME_MODE=webdev` / `TIMEWHERE_WEB_APP_URL`, exposes only native bridge diagnostics/capabilities, has `npm run webdev:desktop:readiness` boundary evidence plus local `npm run webdev:desktop:smoke` evidence, and defaults to the legacy extension shell until Gate E packaging/distribution is approved.
