...
- Current WebDev mutation replay foundation: Worker `/sync/mutations` validates mutation batches, exposes a Task-only replay activation gate plus field-level conflict preview and internal disabled transaction skeleton, persists metadata-only mutation outcomes, and Pages Settings can read sanitized replay diagnostics; replay remains disabled and does not apply user offline writes.
