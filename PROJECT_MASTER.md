...
- Current Task-only replay Phase 1 is implemented as an internal test-only server write contract after Product Owner approval: explicit `test_only_task_replay_enabled` requests can apply safe Task mutations, persist same-field conflicts, reject protected/non-Task mutations, and prove idempotency. User-facing offline writes, Pages queue activation, conflict resolution UI, and production replay remain unapproved.
- Current WebDev sync conflict foundation: D1 `sync_conflicts`, Worker `/sync/conflicts` read APIs, and Pages Settings read-only diagnostics are scaffolded for future offline mutation conflicts; no conflict resolution UI or offline write enablement is approved.
