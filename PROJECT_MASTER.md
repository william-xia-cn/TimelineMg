...
- Current Task-only replay implementation plan is documented in `docs/WEBDEV_OFFLINE_MUTATION_CONFLICT_DESIGN.md`; it is plan-only and requires separate Product Owner approval before any write path or test-only enabled implementation starts.
