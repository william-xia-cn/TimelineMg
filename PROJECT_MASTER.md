...
- Current Desktop Runtime repositioning is opt-in scaffolded with Gate E packet: Electron can load the Web App via `TIMEWHERE_DESKTOP_RUNTIME_MODE=webdev` / `TIMEWHERE_WEB_APP_URL`, exposes only native bridge diagnostics/capabilities, has `docs/WEBDEV_DESKTOP_RUNTIME_GATE_E_READINESS.md` plus `npm run webdev:desktop:readiness` boundary evidence and local `npm run webdev:desktop:smoke` evidence, and defaults to the legacy extension shell until Gate E packaging/distribution is approved.
- Current WebDev Phase 9 / Phase 10 acceptance artifacts: `docs/WEBDEV_PREVIEW_ACCEPTANCE_RUNBOOK.md` defines Gate A preview evidence, preview risk register, and preview rollback/cleanup plan; `docs/WEBDEV_PROD_READINESS_CHECKLIST.md`, `docs/WEBDEV_OBSERVABILITY_BACKUP_RUNBOOK.md`, `npm run webdev:observability:readiness`, `npm run webdev:prod:provision`, `npm run webdev:prod:deploy`, `npm run webdev:prod:acceptance`, `npm run webdev:prod:readiness`, `npm run webdev:prod:package`, `npm run webdev:prod:evidence`, `npm run webdev:prod:evidence:check`, and `npm run webdev:completion:audit` define Gate R internal prod verification, observability/backup readiness, release risk register, rollback package requirements, current completion classification, an evidence package draft that can summarize ignored local status-only evidence, a local status-only evidence runner, and a local evidence summary freshness check without approving public release.
...
- Current Browser Extension WebDev repositioning remains deferred: `docs/WEBDEV_BROWSER_EXTENSION_GATE_D_READINESS.md` and `npm run webdev:extension:readiness` verify the legacy MV3 shell has not become a WebDev replay endpoint or Cloudflare runtime client; Gate D is still required before defining or implementing the first ecosystem phase, CWS submission, or Extension replay.
...
- Current WebDev replay safety foundation: D1 `sync_conflicts`, Worker `/sync/conflicts`, and Pages Settings support single Task conflict review without local-over-cloud writes; `/sync/replay-safety` and Settings expose Phase 4 kill-switch / environment safety evidence with `writes_enabled=false`. Phase 5 adds a Pending Task queue panel with retry preview and local discard actions. Phase 6 records Calendar / Container / Settings replay design only; no non-Task replay implementation is approved. `docs/WEBDEV_TASK_REPLAY_GATE_B_READINESS.md` / `npm run webdev:gate-b:readiness` define the Gate B Task replay approval packet; `docs/WEBDEV_NON_TASK_REPLAY_GATE_C_READINESS.md` / `npm run webdev:gate-c:readiness` define the Gate C non-Task replay approval packet; Task replay and non-Task replay remain gated, and user-facing replay writes remain blocked. Phase 7 adds read-only cross-entity dependency analysis to replay readiness so ordering blockers are visible without enabling writes. Phase 8 hardens Task pending UX without expanding to full offline-first. Phase 9 adds preview readiness hardening with evidence gaps, dependency blockers, required evidence, and approval blockers. D-049 Phase 2-9 is complete; prod release, Calendar/Container/Settings replay implementation, Browser Extension replay, local-over-cloud overwrite, batch conflict handling, and full-entity offline-first still require separate approval.
- Cloudflare dev / preview / prod environments now have an initial scaffold under `workers/` and `pages/`. Gate A dev/preview resource creation is approved and dev/preview D1/R2/KV plus Pages projects have been created/confirmed; true resource ids are stored only in ignored `.wrangler/` local state. Preview Worker and stable Pages preview deploy are live for internal Gate A verification; Google SSO smoke passes on the stable preview origin; `npm run webdev:preview:headers-smoke` verifies stable preview CSP/security headers, root HTML no-store, and hashed asset immutable cache; `npm run webdev:preview:smoke` verifies preview Worker / Pages / D1 / R2 / KV without touching prod; `npm run webdev:preview:core-smoke` verifies preview Account / Structure / Task / Calendar / Settings / Sync / Migration API paths through a temporary smoke account and cleans it up; `npm run webdev:preview:ui-smoke` verifies stable Pages preview UI can read preview Worker data for Dashboard / Tasks / Calendar / Settings through a temporary smoke account and cleans it up; `npm run webdev:preview:data-hygiene-smoke` verifies preview D1 / KV / local temp files have no smoke remnants. `npm run webdev:preview:acceptance` was rerun after the WebDev completion audit / docs cleanup work and passed all preview headers, resource, core API, UI, and data hygiene checks without touching prod or printing tokens/account emails/Cloudflare ids/local private paths. `pages/public/_headers` defines Cloudflare Pages CSP, security response headers, immutable asset cache, and `/` / `index.html` no-store for preview/prod readiness. Gate R internal prod verification is approved and the prod resources have been created/confirmed, with true ids stored only in ignored `.wrangler/` local state. Prod Worker and stable prod Pages deploy are live for internal verification at `https://timewhere-web.pages.dev`; `npm run webdev:prod:acceptance` passes prod headers, Google SSO button render, Worker / Pages / D1 / R2 / KV, core API, UI, and data hygiene checks using only temporary `prod-smoke-*` data. `npm run webdev:prod:evidence -- --run` can generate fresh status-only Gate R evidence for the current clean pushed `WebDev` HEAD, and `npm run webdev:prod:evidence:check` verifies that the ignored local summary matches the current HEAD, command order, exit codes, changed-files sensitive scan, and no-raw-output boundary. Public release, GitHub Release, tag, CWS, Desktop distribution, signing, and notarization remain unapproved.
...
| Container id documentation mismatch | CLOSED | Product&Project Mg / Build&Test | Current authority docs now align to D-013: `tasks`, `containers`, `events`, and `habits` use string UUID / generated string ids at the business/model layer; `plans`, `buckets`, and `labels` may remain numeric planner helper records. Historical Dexie `++id` schema declarations are documented as compatibility details, not a new schema migration. |
