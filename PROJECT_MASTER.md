...
- Current WebDev Phase 9 / Phase 10 acceptance artifacts: `docs/WEBDEV_PREVIEW_ACCEPTANCE_RUNBOOK.md` defines Gate A preview evidence, and `docs/WEBDEV_PROD_READINESS_CHECKLIST.md` plus `npm run webdev:prod:readiness` define Gate R prod readiness without approving deployment or release.
...
- Cloudflare dev / preview / prod environments now have an initial scaffold under `workers/` and `pages/`. Gate A dev/preview resource creation is approved and dev/preview D1/R2/KV plus Pages projects have been created/confirmed; true resource ids are stored only in ignored `.wrangler/` local state. Preview Worker and stable Pages preview deploy are live for internal Gate A verification; Google SSO smoke passes on the stable preview origin; `npm run webdev:preview:smoke` verifies preview Worker / Pages / D1 / R2 / KV without touching prod; `npm run webdev:preview:core-smoke` verifies preview Account / Structure / Task / Calendar / Settings / Sync / Migration API paths through a temporary smoke account and cleans it up; `npm run webdev:preview:ui-smoke` verifies stable Pages preview UI can read preview Worker data for Dashboard / Tasks / Calendar / Settings through a temporary smoke account and cleans it up. Local `npm run webdev:verify`, `npm run webdev:ui:walkthrough`, `npm run webdev:desktop:smoke`, `npm test`, `git diff --check`, sensitive pattern scan, and `npm run webdev:prod:readiness` pass as Phase 9/10 supporting evidence. Prod deploy/release remains unapproved.
