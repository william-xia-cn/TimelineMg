...
- Tasks 已进入 WebDev Task-only queued pending 阶段：通过 Repository 访问 Worker `/tasks`，在线时支持 Cloud-backed 创建、完成/重开、删除、搜索/筛选；离线且已有 Google SSO session 时，Task create/update/complete/reopen 写入本地 pending queue 并标记待同步，delete 仍需联网；Settings 可查看 pending queue、执行 retry preview、或 discard local pending。
