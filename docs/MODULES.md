...
> Current baseline note (2026-05-15): Internal MVP is accepted and baseline-stabilized as a local-first MVP. MatrixView import, ManageBac mapping/task-sync, and Task Date Arrange are active baseline stabilization features. D-019 approves optional Google data sync planning, but product code implementation has not started. Background alarm automation, system reminder notifications, Chrome Web Store submission, and public release are not current-scope features.
...
| **Google 数据同步** | 可选连接 Google 账号进行云端持久化和跨终端同步 | 不登录也可完整使用；第一阶段规划为手动双向同步 |
...
- 第一版从手动双向同步和冲突确认开始，不做 background alarm。
...
│ Google 数据同步                                                              │
├──────────────────────────────────────────────────────────────────────────────┤
│ 状态: 本地模式 / Google 同步已连接 / 需要重新授权 / 有冲突待处理             │
│ [连接 Google 同步] [立即同步] [从 Google 恢复] [上传本设备数据] [断开同步]   │
│                                                                              │
│ 说明: 不连接 Google 也可以完整使用 TimeWhere。Google 仅用于云端备份和跨设备。 │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
1. Auth-only spike：Chrome Identity / OAuth token 获取，Settings 显示连接状态，不做数据同步。
2. Snapshot export/import：定义 canonical JSON，本地导出 / 恢复。
3. Drive `appDataFolder` 手动备份 / 恢复。
4. 双向同步 v1：本地与云端 snapshot 合并，冲突进入确认页。
5. 自动触发策略：后续再评估 Dashboard / Settings 入口检查；不做 background alarm。
