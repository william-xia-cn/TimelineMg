...
> Current baseline note (2026-05-22): Internal MVP is accepted and baseline-stabilized as a local-first MVP. MatrixView import, ManageBac mapping/task-sync, Task Date Arrange, optional Google data sync v1, local system task reminders, and Chrome Side Panel toolbar surface are active baseline stabilization features. The submitted CWS package is pending review and the current source has moved ahead; any further Chrome Web Store package submission or public release is not current-scope without fresh Product Owner approval.
...
左侧导航栏 + 水平排列的 4 个数据区。当前桌面主布局采用“三主列 + 独立消息流”模型：
...
也就是 Dashboard 首屏三主列比例为 `2:1.5:1`，当前任务、日程（今日 & 明日）、本周进度恰好填满首屏；消息流是独立列，位于右侧并通过横向滚动可见。本周进度和消息流不能被合并压缩成同一个区域。
...
| 第2列 日程 | 1.5份 | containers + events（今日&明日）|
| 第4列 本周进度 | 1份 | tasks（本周统计 + 重点列表）|
| 第5列 消息流 | 固定280px | tasks + events（计算得出）|
...
│  生效日期:  [2026-04-10] 至 [2026-05-10] │ ← 容器/日程事件重复生效范围
│  ☐ 全天事件                         │  ← 仅"日程事件"显示
...
**生效日期范围**：时间容器和日程事件表单均包含 `active_start_date` / `active_end_date`。Calendar 创建入口默认开始为点击/事件日期、结束为开始日期后 1 个月；编辑旧记录时空值保持无边界。保存时若两个日期都有值，结束日期不能早于开始日期。该范围只限制重复对象出现的自然日期，不改变每天的开始/结束时间；`repeat='none'` 日程事件仍只在 `date` 当天显示，container override/skip 仍按 `date` 精确匹配。
...
Settings 添加时间容器时默认 `active_start_date = 今天`、`active_end_date = 今天后 1 个月`；容器列表摘要显示时间、重复规则和生效日期范围，缺少结束日期时显示长期。
...
Side Panel / Popup 的 `当前任务` 必须与 Dashboard `当前任务` 列保持同一数据投影、列表结构和操作语义。Side Panel 是 Dashboard 当前任务列的持久紧凑投影，不能降级成只显示单个任务卡。
...
| **快捷入口** | 打开 Dashboard / Task Board / Calendar / Settings | 底部四宫格按钮 |
| **临时添加任务** | 在侧边栏内打开任务详情式创建面板 | 默认 English Plan / `作业` Bucket，可编辑完整任务字段 |
| **今日总结** | 在侧边栏内打开 Daily Journal 审阅/草稿/提交面板 | 保存草稿或提交总结 |

Side Panel / Popup 当前任务规则：
...
- Side Panel / Popup 任务卡展开后必须能完整显示当前任务的主要信息和操作区，不能因为固定高度、`max-height`、`overflow:hidden`、行数裁剪或容器滚动策略导致 notes、metadata、操作按钮、延后行显示不全。过长文本可换行或在卡片内部自然扩展；外层可以滚动。
- 无任务时显示 `暂无待办任务`，不能再写成 `暂无进行中的任务`，因为 Popup 使用的是 Daily Settle 当前任务投影，不是只读取 `in_progress`。
- ManageBac 来源任务在 Side Panel / Popup 中同样显示 `延后    ManageBac 来源任务不能延后`，不显示 1/3/7 天延后按钮。
- Side Panel 的临时任务入口使用当前日期作为默认 `start_date` / `due_date`，优先选择名称或学科包含 `English` / `英文` 的 Plan，并确保该 Plan 具备学科默认 Bucket 模板；默认 Bucket 为 `作业`。用户仍可修改 Plan、Bucket、日期、定时时间、时长、优先级、进度、Checklist、周期任务、Label 和 Notes。
- Side Panel 的今日总结入口使用 Daily Journal draft/snapshot 数据，支持保存草稿和提交总结；它不得要求用户跳转到完整 Dashboard 才能完成日终复盘。
- Dashboard 当前任务列也提供同样的“未计划的任务添加”卡片；点击后在当前任务列中打开任务详情式创建面板，而不是打开普通 modal 或只创建缺少字段的任务。

### 5.4 尺寸约束

- Popup 兼容页固定约 **360×560px**。
- Side Panel 使用 `100vw × 100vh`，最小宽度 320px，适配 Chrome 侧边栏宽度。
- Side Panel 底部导航固定在底部；任务列表、临时任务卡、今日总结卡和详情 modal 内部滚动，避免按钮被裁剪。
...
| 场景 | 通信方式 |
|------|----------|
| Side Panel / Popup → Page | `chrome.tabs.create()` |
...
**最后更新**: 2026-05-22 (Side Panel / quick-add documentation sync)
**版本**: v2.3
