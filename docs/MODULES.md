...
> Current baseline note (2026-05-14): Internal MVP is accepted and baseline-stabilized as a local-first MVP. MatrixView import, ManageBac mapping/task-sync, and Task Date Arrange are active baseline stabilization features. Google Sync/OAuth, background alarm automation, system reminder notifications, Chrome Web Store submission, and public release are not current-scope features.
