...
|---|---|---|---|
| Google SSO account session | Implemented local | Worker `/auth/google`、`/auth/session/refresh`、`/account/me`、`/account/profile`、`/account/status`；Pages account panel、workspace profile、runtime/gate status。 | 真实 dev/preview OAuth 配置需要 Gate A。 |
...
| Automatic migration | Implemented local scaffold | Legacy snapshot adapter；Worker `/migration/runs`；R2 raw snapshot；D1 import；conflict review。 | 真实 legacy DB smoke and preview cloud evidence need Gate A. |
...
| Sync change feed | Implemented local | D1 `sync_changes`、Worker `/sync/changes`、Pages Settings `Refresh changes`、`npm run webdev:ui:walkthrough` creates a post-bootstrap Cloud Task and verifies it reaches Tasks UI through cursor refresh。 | Preview cloud cursor validation after Gate A。 |
...
- prod deployment；
-真实 Cloudflare dev / preview 资源创建；
- Task replay 写 Cloud；
...
- 本清单所有非 gate、非 deferred capability 至少达到 `Implemented local`；
