...
**状态**: Gate R internal prod verification checklist
**适用阶段**: Phase 10 internal prod verification
**边界**: Product Owner 已批准创建/确认 prod Cloudflare resource，并部署 prod Worker / Pages 用于内部验证；本文不批准 public release、GitHub Release、tag、merge、CWS、签名、公证或分发。

TimeWhere WebDev v1 的 Gate R 本段目标是快速进入可验证的 prod 环境，证明 Cloud-first / Web-first 架构可以在真实 prod 资源上完成内部 smoke。正式公开发布仍必须由 Product Owner 另行批准。
...
- 核心业务 smoke 通过。
- 数据备份/回滚策略已审查。
- `docs/WEBDEV_OBSERVABILITY_BACKUP_RUNBOOK.md` 已审查，`npm run webdev:observability:readiness` 通过。
- 安全与隐私核查通过。
...
- `docs/WEBDEV_OBSERVABILITY_BACKUP_RUNBOOK.md` 已完成并通过静态检查。
...
- Prod provision / deploy evidence:
- Prod smoke / core / UI / data hygiene:
- Google SSO prod origin evidence:
...
## 15. Internal Prod Verification Commands

Product Owner 已批准本段内部 prod verification，可运行：

```powershell
npm run webdev:prod:provision
npm run webdev:prod:deploy
npm run webdev:prod:headers-smoke
npm run webdev:prod:sso-smoke
npm run webdev:prod:smoke
npm run webdev:prod:core-smoke
npm run webdev:prod:ui-smoke
npm run webdev:prod:data-hygiene-smoke
...
`webdev:prod:provision` 创建/确认 prod Worker、Pages、D1、R2、KV，并只把真实 resource id 写入 ignored `.wrangler/` 本地状态。

`webdev:prod:deploy` 应用 prod D1 migrations，部署 prod Worker，构建并部署 prod Pages。它不 tag、不 GitHub Release、不 CWS、不 Desktop package/signing/distribution。

prod smoke 命令只使用临时 `prod-smoke-*` / `prod-ui-smoke-*` 测试账户和 session；不迁移真实用户数据，不记录 token、cookie、OAuth secret、真实账号邮箱、Cloudflare API token 或本地私密路径。

`webdev:prod:readiness` 检查仓库中的 readiness 文档、prod script availability、`workers/wrangler.toml` prod placeholder、env example、Gate R 边界、replay kill switch 和 secret hygiene。它不会创建 Cloudflare prod resource、不会部署 Worker/Pages、不会 tag、不会 release、不会修改远端状态。
...
`webdev:prod:package` 只把当前分支、commit、Gate 状态、必跑命令可用性、ignored 本地 status-only evidence summary 新鲜度、已知限制和回滚摘要输出为内部验证 evidence 草稿。命令清单中的 `[x]` 只表示脚本入口存在，不表示该命令已针对当前 commit 重新执行；Fresh Local Evidence Summary 才表示当前 clean pushed HEAD 是否已有通过的 status-only 证据。它只读取 `.wrangler/webdev-gate-r-evidence-summary.json` 的状态摘要，不读取真实 Cloudflare resource state、不写入真实 Cloudflare resource id，不能替代公开发布批准。

`webdev:prod:evidence` 默认只输出 Gate R 证据命令计划，不执行命令。需要生成本地执行摘要时使用 `npm run webdev:prod:evidence -- --run`；该模式会要求工作树干净且 `HEAD == origin/WebDev`，顺序运行 Gate R 证据命令，并只把命令、退出码、耗时、branch/commit/upstream sync 状态写入 ignored `.wrangler/webdev-gate-r-evidence-summary.json`，不保存原始命令输出，不发布，不启用 replay。

`webdev:prod:evidence:check` 只读取 ignored `.wrangler/webdev-gate-r-evidence-summary.json`，验证它对应当前 clean 且已推送的 `WebDev` HEAD、`upstream_synced=true`、命令顺序完整、所有命令退出码为 0、changed-files scan 为 0、且摘要中没有 `stdout` / `stderr` / raw output 字段。它不发布、不启用 replay；若摘要来自旧 commit，会要求先重新运行 `npm run webdev:prod:evidence -- --run`。
