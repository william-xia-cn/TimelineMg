...
| 6 | Enable offline queue for Task only. | Offline create/update/complete replay tests and conflict tests pass after Product Owner approval. |
| 7 | Add Calendar and Structure entities. | Entity-specific conflict tests pass. |
| 8 | Add Settings and cross-entity relationship validation. | Settings/runtime boundary tests pass. |
| 9 | Promote conflict UI from diagnostics to user workflow. | Manual merge / keep cloud / apply local acceptance tests pass. |
...
1. Add disabled Task replay outcome persistence design hooks.
