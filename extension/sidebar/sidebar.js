...
let sidebarPartialCompleteReopenTaskId = null;
let sidebarCurrentTaskExpandedTaskId = null;
...
function renderCurrentTaskList(tasks) {
    ...
    const anchoredIndex = sidebarCurrentTaskExpandedTaskId
        ? tasks.findIndex(task => String(task.id) === String(sidebarCurrentTaskExpandedTaskId))
        : -1;
    if (sidebarCurrentTaskExpandedTaskId && anchoredIndex < 0) {
        sidebarCurrentTaskExpandedTaskId = null;
    }
...
function renderCurrentTaskCard(task, index, expandedIndex = 0) {
    ...
    const statusLabel = getSidebarTaskStatusLabel(task.progress);
    const checklistHtml = renderSidebarTaskChecklist(task);
...
function getSidebarTaskStatusLabel(progress) {
...
function renderSidebarTaskChecklist(task) {
    const checklist = Array.isArray(task.checklist) ? task.checklist : [];
...
async function renderSidepanelBottomActions() {
    ...
    if (TimeWhereDB.ensureDailyJournalSnapshot) {
        await TimeWhereDB.ensureDailyJournalSnapshot(todayStr, now).catch(error => {
            console.warn('[Sidebar] Daily journal snapshot skipped:', error);
        });
    }
...
function setupEventListeners() {
    ...
    document.addEventListener('change', handleSidebarDelegatedChange);
...
function handleTaskCardToggle(event) {
    ...
    if (card.dataset.taskCardId) {
        sidebarCurrentTaskExpandedTaskId = String(card.dataset.taskCardId);
    }
    const taskList = document.getElementById('currentTaskList');
...
async function handleTaskActionClick(event) {
    ...
    if (actionEl.dataset.action === 'open-external-link') {
        await runSidebarAction(actionEl, async () => openTaskNotesExternalLink(actionEl));
        return;
    }
    ...
    if (actionEl.dataset.action === 'save-sidepanel-quick-add-task') {
        await runSidebarAction(actionEl, saveSidepanelQuickAddTask);
        return;
    }
    ...
    if (actionEl.dataset.action === 'save-daily-journal-draft') {
        await runSidebarAction(actionEl, async () => saveDailyJournalFromModal(false));
        return;
    }
    if (actionEl.dataset.action === 'submit-daily-journal') {
        await runSidebarAction(actionEl, async () => saveDailyJournalFromModal(true));
        return;
    }
    if (actionEl.dataset.action === 'toggle-partial-complete-menu') {
        toggleSidebarTaskPartialCompleteMenu(actionEl.dataset.taskId);
        return;
    }
    if (actionEl.dataset.action === 'partial-complete-ratio') {
        await runSidebarAction(actionEl, async () => saveSidebarTaskPartialCompleteRatio(
    ...
    if (actionEl.dataset.action === 'save-current-task-detail') {
        await runSidebarAction(actionEl, saveCurrentTaskDetailModal);
        return;
    }
    if (actionEl.dataset.action === 'toggle-defer-menu') {
        toggleSidebarTaskDeferMenu(actionEl.dataset.taskId);
        return;
    }

    await runSidebarAction(actionEl, async () => {
        ...
        if (action === 'toggle-popup-checklist') await toggleSidebarTaskChecklist(taskId, actionEl.dataset.checklistId);
    });
}

async function handleSidebarDelegatedChange(event) {
    const checklistActionEl = event.target.closest('[data-action="toggle-partial-complete-checklist"]');
    if (checklistActionEl) {
        await runSidebarAction(checklistActionEl, async () => saveSidebarTaskPartialCompleteChecklistItem(
...
async function runSidebarAction(control, action) {
    ...
    try {
        await action();
        await reloadSidebar();
        if (sidebarPartialCompleteReopenTaskId) {
            const taskId = sidebarPartialCompleteReopenTaskId;
            sidebarPartialCompleteReopenTaskId = null;
            requestAnimationFrame(() => reopenSidebarTaskPartialCompleteMenu(taskId));
        }
    } catch (error) {
...
async function toggleSidebarTaskChecklist(taskId, checklistId) {
    const task = await TimeWhereDB.getTaskById(taskId);
...
function toggleSidebarTaskDeferMenu(taskId) {
    ...
    closeSidebarTaskPartialCompleteMenus();
    ...
    if (shouldOpen) {
        sidebarCurrentTaskExpandedTaskId = String(taskId);
        targetMenu.removeAttribute('hidden');
        targetButton.setAttribute('aria-expanded', 'true');
    } else if (sidebarCurrentTaskExpandedTaskId === String(taskId)) {
        sidebarCurrentTaskExpandedTaskId = null;
    }
}

function closeSidebarTaskPartialCompleteMenus() {
...
function reopenSidebarTaskPartialCompleteMenu(taskId) {
...
function toggleSidebarTaskPartialCompleteMenu(taskId) {
    const escapedTaskId = CSS.escape(String(taskId));
    const targetMenu = document.querySelector(`[data-partial-complete-menu-for="${escapedTaskId}"]`);
    const targetButton = document.querySelector(`[data-action="toggle-partial-complete-menu"][data-task-id="${escapedTaskId}"]`);
    if (!targetMenu || !targetButton) return;
...
async function saveSidebarTaskPartialCompleteRatio(taskId, percent) {
    sidebarCurrentTaskExpandedTaskId = String(taskId);
    const task = await TimeWhereDB.getTaskById(taskId);
    if (!task) {
        showToast('任务不存在或已删除', 'error');
...
async function saveSidebarTaskPartialCompleteChecklistItem(taskId, checklistId, checked) {
    sidebarCurrentTaskExpandedTaskId = String(taskId);
    const task = await TimeWhereDB.getTaskById(taskId);
    if (!task) {
        showToast('任务不存在或已删除', 'error');
    ...
    await TimeWhereDB.updateChecklist(taskId, checklist);
    sidebarPartialCompleteReopenTaskId = String(taskId);
    showToast('部分完成已更新', 'success');
}
