...
function getContainerLayer(c) {
    ...
    if (c.name && c.name.includes('学习')) return 1;
    return 2;
}
 ...
 * @param {Date}  now - 当前时间
 * @returns {{ result, activeContainer, containerInfo, currentTasks, unassigned, sortedPool }}
 */
function dailySettle(taskPool, todayContainers, now) {
    ...
    allSorted.forEach(c => {
        const cap = timeToMinutes(c.time_end) - timeToMinutes(c.time_start);
...
async function ensureDefaultContainers() {
    ...
    const c1 = await TimeWhereDB.addContainer({
        name: '自由时间',
        color: '#7B68EE',
        time_start: '15:30',
        time_end: '18:30',
    ...
    await TimeWhereDB.updateContainer(c1.id, { layer: 2 });

    const c2 = await TimeWhereDB.addContainer({
        name: '学习时间',
        color: '#4A90D9',
        time_start: '18:30',
        time_end: '21:30',
    ...
    await TimeWhereDB.updateContainer(c2.id, { layer: 1 });

    const c3 = await TimeWhereDB.addContainer({
        name: '自由时间',
        color: '#7B68EE',
        time_start: '21:30',
        time_end: '22:30',
    ...
    await TimeWhereDB.updateContainer(c3.id, { layer: 2 });
