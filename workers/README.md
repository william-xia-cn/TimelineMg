...
- `/sync/mutations` 只有 disabled/internal contract skeleton：校验 mutation batch，定义 Task-only replay activation gate 和字段级冲突预判，记录 metadata-only outcome，并拒绝 replay，不应用任何用户离线写入。
