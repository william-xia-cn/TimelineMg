...
- `POST /sync/mutations/dry-run` 提供 internal disabled dry-run：复用 gate / transaction skeleton，并关联已有 outcome / conflict 记录；对 conflict candidate 返回不落库的 sanitized conflict preview；不写入、不创建 conflict、不应用用户离线写入。
